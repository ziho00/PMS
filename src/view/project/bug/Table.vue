  ...
  <a-card class="page-card">
    ...
    >
      ...
      <template #status="{ text }">
        <StateTag mode="bug" :value="text" />
      </template>
      <template #priority="{ text }">
        <PriorityTag mode="bug" :value="text" />
      </template>
...
export default {
  ...
  setup() {
    ...
    const handleCreateItem = () => {
      ...
      router.push(`/${projectId}/bug/edit/create`);
    };

    // 编辑
    const handleEdit = (record) => {
      const { projectId } = route.params;
      router.push(`/${projectId}/bug/edit/${record.bug_id}`);
    };

    // 删除
    const handleDeleteItem = (record) => {
      Modal.confirm({
        title: `确定删除缺陷【ID:${record.bug_id}】?`,
